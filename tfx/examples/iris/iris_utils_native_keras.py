...
def run_fn(fn_args: TrainerFnArgs):
  ...
  features_spec = dict(
      sepal_length=tf.TensorSpec(shape=(None, 1), dtype=tf.float32),
      sepal_width=tf.TensorSpec(shape=(None, 1), dtype=tf.float32),
      petal_length=tf.TensorSpec(shape=(None, 1), dtype=tf.float32),
      petal_width=tf.TensorSpec(shape=(None, 1), dtype=tf.float32))
  signatures = {
      'serving_default':
